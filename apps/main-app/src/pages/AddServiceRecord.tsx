...
export default function AddServiceRecord() {
  ...
  const addOperation = () => {
    ...
    const newOperation: ServiceOperation = {
      id: IdGenerator.generateServiceOperationId(),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Save using DataService (which will use IdGenerator for ID generation)
      await DataService.saveServiceRecord(serviceRecordData)
      
      // Navigate back
      navigate('/')
    } catch (error) {
      console.error('Error saving service record:', error)
      alert('Ошибка при сохранении записи о сервисе')
    }
  }
