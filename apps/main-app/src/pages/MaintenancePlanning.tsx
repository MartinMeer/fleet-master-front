...
export default function MaintenancePlanning() {
  ...
  const [plannedDate, setPlannedDate] = useState(dateUtils.getCurrentRussianDate())
  const [plannedCompletionDate, setPlannedCompletionDate] = useState(dateUtils.getCurrentRussianDate())
  ...
  const loadPlanForEditing = (plan: MaintenancePlan) => {
    ...
    if (selectedCar) {
      ...
      setIsLoadingPlan(false) // Reset flag on error
      return
    }

    // Load repair operations - create items for all saved repair operations
    console.log('Plan repair operations:', plan.repairOperations)
    const repairOperations = plan.repairOperations.map(repairOp => ({
      ...
      estimatedCost: repairOp.estimatedCost || 0,
      notes: repairOp.notes || '',
      selected: true, // All saved repair operations should be selected
      planNotes: repairOp.notes || ''
    }))
    console.log('Loaded repair items:', repairOperations)
    setRepairItems(repairOperations)
  ...
  const saveServiceShop = () => {
    ...
    const shopData: ServiceShop = {
      id: editingShopId || IdGenerator.generateId('shop'),
      name: shopName.trim(),
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {planId ? 'Просмотр существующего плана обслуживания (только для чтения)' : 'Составление плана периодического и ремонтного обслуживания'}
                </p>
              </div>
            </div>
            <Button 
      ...
      <main className="px-4 py-6">
        ...
        <Card className="mb-6">
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <Select value={selectedCarId} onValueChange={setSelectedCarId} disabled={!!planId}>
                <SelectTrigger>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <DateInput
                  ...
                  onChange={(value) => setPlannedCompletionDate(value)}
                  disabled={!!planId}
                />
              </div>
              <div>
        ...
        <Dialog open={isCloseConfirmOpen} onOpenChange={isSaving ? undefined : setIsCloseConfirmOpen}>
          <DialogContent>
            <DialogHeader>
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end space-x-2">
                <Button variant="outline" onClick={() => setIsCloseConfirmOpen(false)} disabled={isSaving}>
                  Отмена
                </Button>
                <Button variant="outline" onClick={handleCloseWithoutSaving} disabled={isSaving}>
                  Закрыть без сохранения
                </Button>
                <Button onClick={handleSaveAndClose} disabled={isSaving} className="bg-blue-600 hover:bg-blue-700">
                  <Save className="h-4 w-4 mr-2" />
